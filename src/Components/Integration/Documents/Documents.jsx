...
const Documents = () => {
  ...
  const handleSubmit = async () => {
    try {
      const response = await postApi("/Documents/Add", {
   ...
   const handleEditSubmit = async () => {
    try {
      const response = await postApi("/Documents/Update", {
        entity_type: "integration",
        entity_id: 9,
  ...
  const deleteData = async (id) => {
    try {
      await deleteApi("/Documents/Delete/" + id);
      fetchData();
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await getApi("/Documents/GetList");
