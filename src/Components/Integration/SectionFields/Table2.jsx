...
const Table2 = ({ activeSection }) => {
  ...
  const handleSubmit = async () => {
    try {
      const response = await postApi("/SectionFields/Add", {
        ...values,
        entity_type: "section",
        section: "section",
        entity_id: activeSection?.id,
      });
      if (response.status) {
        toast.success("Field added successfuly");
        fetchSectionFields(activeSection?.id);
        setValues({
          accountType: "",
          fieldType: "",
          schemaLayout: "",
    ...
    try {
      const response = await postApi("/SectionFields/Update", {
        ...editValues,
        entity_type: "section",
        section: "section",
        entity_id: activeSection?.id,
  ...
  const fetchSectionFields = async (id) => {
    setLoading(true);
    try {
      const response = await getApi(
        "/SectionFields/GetBySection/section/" + id
      );
      if (response.status) {
        ...
        <div className="bg-white  rounded-lg shadow-lg ">
          <div className="flex justify-between items-center px-6 py-4 border-b-[2px] border-border_color">
            <span>
              <h2>
                Section Fields{" "}
                {activeSection?.title && "for " + activeSection?.title}
              </h2>
            </span>
            {activeSection?.id !== 0 && (
          ...
          <div className="lg:overflow-x-auto xl:overflow-x-hidden  hidden py-4  lg:block px-10  w-full">
           ...
           <Popup
                isOpen={isEditOpen}
                heading={"Update field information"}
                ...
                content={
                    ...
                    <div className="flex-1 mt-6 relative border-[1.9px] border-gray-600 px-4 py-2 rounded">
                      ...
                      <select
                        name="accountType"
                        onChange={(e) => setEditValues({...editValues, schema: e.target.value})}
